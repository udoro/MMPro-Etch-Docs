...
You are here because the entry file put you on the **build path**. Read this file in full, then
work through the steps below in order.

### 0. Skill File First
1. **Read this entire file before writing or executing any edits — no exceptions for length.** You are on the build path; that is what this file is for. (A correction is handled entirely from the entry file and never reaches this step.) This file is long enough that a single `Read` call will return a `[Truncated: PARTIAL view]` notice instead of the full content. If you see that notice, Step 0 is NOT complete — continue reading with `offset`/`limit` until you reach the final line and no truncation notice remains. Do not proceed to Step 1 (Connect & Preflight Gate) on a partial read, even if the visible portion looks sufficient for the task at hand.
2. **File read confirmation (mandatory on the build path):** Before proceeding past Step 0, state in chat: `"Skill file read: lines 1-[N] of [N], complete."` On the correction path, state instead: `"Correction path: read [sections], skipped the full read."` (substitute the real total line count). If you cannot truthfully make that statement, you have not satisfied Step 0 — go back and finish reading.
...
Default values omitted: YES — [props left alone because the block is already unset or already correct; a prop whose stored value differs must be written even if the target is the defaultault, or N/A]
...
- A local prop is being set when a working global equivalent exists (trigger mode is exempt: its global is inert)
...
> Run the fingerprint check from the entry file before any prop work, and dump a schema only for a
> component whose fingerprint disagrees with Ref 4.
...
```js
// inspect-schema.js — no IIFE. The connector already runs this body as an async
// function, so an IIFE returns an unresolved Promise and the eval yields null.
const componentJson = etch.components.getJson(RESOLVED_ID);
return JSON.stringify(componentJson.properties);
```
*(`RESOLVED_ID` is the id you resolved by NAME. Never paste an id from these docs: they are
install-local, and three of the five placeholders in older copies are wrong on a real install.)*
...
6. **Verify** — read the changed prop/variable back (configuration), then check the published page (rendered). See "Visual verification". Always state which level you reached.
...
> Saving before verifying, the three-check ladder for an editor/page disagreement, and the
> correction path are all in the entry file. They apply here too.
...
### Do not write a prop whose stored value already gives you what you want
...
* **To hide/fade a breakout dropdown's icon, target the toggle BUTTON inside it, not the dropdown `<li>` wrapper — and remember `.dwc-open` only fires for the MOBILE MENU, not for a breakout dropdown opening its own panel.** Two separate mistakes here:
  ...
  ```
* **Check a style entry's CSS before reusing it by selector — an entry can outlive the blocks that used it.** Deleting a panel does not delete its style entries, so a site that has hosted an earlier build of the same design still has every one of its selectors. A create-if-missing helper (`list().find(s => s.selector === sel)?.id ?? create(...)`) then silently *reuses* those entries, and the new panel inherits the old CSS: the build reads back perfectly (the class and `styles[]` are both correct) and renders with values nobody wrote this session. Before reusing a matched entry, read its `css` and compare it against what you intended; overwrite with `styles.update(id, { css })` when it differs. The same applies to `.dwc-*-vars`: an earlier session's overrides sit in those entries, so read the block before adding a declaration you may already have.
* **A style entry can hold two blocks with the identical scope selector, and the later one silently wins.** Appending `html:not(.dwc-mobile) & { ... }` to an entry that already contains an `html:not(.dwc-mobile) & { ... }` block does not merge them — both are emitted, and any property set in both resolves to the last. The write succeeds, `styles.list()` shows your CSS present, and the rendered value is the other block's. Symptom: a change that reads back correctly and does nothing at all. Before appending a scoped block, grep the entry's existing css for that same scope and extend the existing block instead of adding a second one. Verify with computed style on the published page, never with a read-back.
