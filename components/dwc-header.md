...
Creates an immersive, glass-like digital backdrop where on-screen elements dynamically bend, refract, and morph in response to motion. Mutually exclusive with **Header Blur** — enabling Liquid Glass suppresses the header's `backdrop-filter` blur rule on both desktop and mobile.

| Setting            | Description                                                                                                                                                                              |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Enable**          | Turns on the liquid-glass effect on the header. When active, this replaces the standard Header Blur backdrop-filter — the two are mutually exclusive.                            |
...
| **Header Blur**             | Applies a `backdrop-filter` blur to the header bar, creating a frosted glass effect. Controls `--header-blur`. Default: `10px`.                                                                       |
